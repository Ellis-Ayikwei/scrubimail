    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            try:
                ...
                logger.exception(
                    "Plan upgrade initialize failed user_id=%s plan_id=%s",
                    uid,
                    plan.id,
                )
                return Response({"error": str(e)}, status=status.HTTP_400_BAD_REQUEST)
        ...
        logger.warning(
            "Plan upgrade validation failed user_id=%s payload=%s errors=%s",
            uid,
            payload,
            serializer.errors,
        )
...
class PaymentVerificationView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            billing_service = BillingService()
            payload = billing_service.handle_payment_verification(
                serializer.validated_data["reference"],
                request.user,
            )
            status_code = (
                status.HTTP_200_OK if payload.get("ok") else status.HTTP_400_BAD_REQUEST
            )
...
class StartTrialView(APIView):
    ...
    def post(self, request):
        ...
        if success:
            ...
            return Response(
                {"error": "Failed to start trial"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        return Response(
            {
                ...
                "current_plan": (
                    profile.current_plan.name if profile.current_plan else None
                ),
            }
        )
...
class RateLimitStatusView(APIView):
    ...
    def get(self, request):
        ...
        if not plan:
            return Response(
                {"error": "No plan found"}, status=status.HTTP_404_NOT_FOUND
            )
        ...
        return Response(
            {
                ...
                "usage": {
                    ...
                    "percentage_used": round(
                        (
                            (requests_made / plan.max_api_calls_per_hour * 100)
                            if plan.max_api_calls_per_hour > 0
                            else 0
                        ),
                        2,
                    ),
                ...
                "reset": {
                    ...
                    "reset_time": (
                        timezone.now() + timedelta(seconds=seconds_until_reset)
                        if seconds_until_reset > 0
                        else None
                    ),
                },
                "warnings": self._get_warnings(
                    requests_remaining, plan.max_api_calls_per_hour
                ),
            }
        )
    ...
    def _get_warnings(self, remaining, limit):
        ...
        if percentage_remaining <= 10:
            warnings.append(
                "You have used 90% of your hourly rate limit. Consider upgrading your plan."
            )
...
class ListCreditPackagesView(APIView):
    ...
    def get(self, request):
        ...
        packages = CreditPackage.objects.filter(is_active=True).order_by(
            "sort_order", "price"
        )
...
class CreditPackageDetailView(APIView):
    ...
    def get(self, request, package_id):
        ...
        serializer = CreditPackageSerializer(package, context={"request": request})
...
class PurchaseCreditPackageView(APIView):
    ...
    def post(self, request):
        ...
        promo_code_str = (request.data.get("promo_code") or "").upper()
        ...
        if promo_code_str:
            try:
                ...
                is_valid, message = promo_code.is_valid(
                    user=request.user, package=package, amount=original_amount
                )
...
class CompleteCreditPackagePurchaseView(APIView):
    ...
    def post(self, request, purchase_id):
        ...
        purchase = get_object_or_404(
            CreditPackagePurchase, id=purchase_id, user=request.user
        )
...
class ExpiringCreditsView(APIView):
    ...
    def get(self, request):
        ...
        transactions = CreditTransactionSerializer(
            expiring_info["transactions"], many=True
        ).data
        ...
        return Response(
            {
                ...
                "warnings": self._get_expiry_warnings(
                    expiring_info["total_credits"], expiring_info["days_until_expiry"]
                ),
            }
        )
        ...
        return Response(
            {
                ...
                "warnings": self._generate_warnings(
                    expiring_data["total_credits"], expiring_data["days_until_expiry"]
                ),
            }
        )
    ...
    def get(self, request):
        ...
        total_used = abs(
            CreditTransaction.objects.filter(
                billing_profile=profile, transaction_type="usage", amount__lt=0
            ...
            if amount:
                ...
                discount_info = {
                    ...
                    "savings_percentage": float(
                        (discount_amount / amount * 100) if amount > 0 else 0
                    ),
                }
...
class RedeemPromoCodeView(APIView):
    ...
    def post(self, request):
        ...
        # Validate promo code
        is_valid, message = promo_code.is_valid(
            user=request.user, plan=plan, package=package, amount=original_amount
        )
...
class ListPromoCodesView(APIView):
    ...
    permission_classes = [AllowJWTOrAPIKey]
    permission_required = ["billing.add_promocode"]
    ...
    def post(self, request):
        ...
        logger.error(f"Error creating promo code: {serializer.errors}")
...
class PromoCodeRedemptionHistoryView(APIView):
    ...
    def get(self, request):
        ...
        redemptions = (
            PromoCodeRedemption.objects.filter(user=request.user)
        ...
        return Response(
            {
                ...
                "recommendations": {
                    ...
                    "upgrade_suggestion": notification_service._get_upgrade_suggestion(
                        profile
                    ),
                },
            }
